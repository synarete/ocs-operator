...
func (obj *ocsStorageQuota) ensureCreated(r *StorageClusterReconciler, sc *ocsv1.StorageCluster) error {
	for idx, op := range sc.Spec.Overprovision {
		...
		if err != nil {
			...
			_, err := r.QuotaV1.ClusterResourceQuotas().Create(context.TODO(), storageQuota, metav1.CreateOptions{})
			if err != nil {
...
func (obj *ocsStorageQuota) ensureDeleted(r *StorageClusterReconciler, sc *ocsv1.StorageCluster) error {
	for idx := range sc.Spec.Overprovision {
		...
		_, err := r.QuotaV1.ClusterResourceQuotas().Get(context.TODO(), quotaName, metav1.GetOptions{})
		if err == nil {
			r.Log.Error(err, fmt.Sprintf("Delete ClusterResourceQuota %s", quotaName))
			err = r.QuotaV1.ClusterResourceQuotas().Delete(context.TODO(), quotaName, metav1.DeleteOptions{})
			if err != nil {
...
func hardLimitOf(sc *ocsv1.StorageCluster, op *ocsv1.OverprovisionSpec) *resource.Quantity {
	if op.Capacity != nil {
		return op.Capacity
	}
	if op.Percentage > 0 {
...
func calcUseableCapacity(sc *ocsv1.StorageCluster) int64 {
	...
	for _, ds := range sc.Spec.StorageDeviceSets {
		...
		if ok {
			...
			useableCapacity += int64(storageQuantity.AsApproximateFloat64()) * int64(count) * int64(replica)
		}
	}
